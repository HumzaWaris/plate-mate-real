...
function getTop3ClosestHalls(
  userLocation: [number, number]
): Array<[string, number]> {
  const distances: Array<[string, number]> = Object.entries(hall_locations).map(
    ([hall, coords]) => {
      const [lat, lon] = coords;
      return [hall, haversine(userLocation[0], userLocation[1], lat, lon)] as [
        string,
        number
      ];
    }
...
function filterItemsByHalls(
  items: any[],
  halls: Array<[string, number]>
): any[] {
  const allowedHalls = halls.map(([hall]) => hall);
  console.log(allowedHalls)
  console.log(items.filter((item) => allowedHalls.includes(item.dining_hall ?? "")))
  return items.filter((item) => allowedHalls.includes(item.dining_hall ?? ""));
}

function calculateDailyCalories(weight: number): number {
  return weight * 15;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const daily_calories = calculateDailyCalories(weight);
    ...
    // Retrieve meal items concurrently
    ...
    const breakfast_filtered = filterItemsByHalls(
      breakfast_items,
      closest_halls
    );
